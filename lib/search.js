 ...
 * 4. 每个结果附带上下文预览
 */
import { cosineSimilarity, computeNorm } from "./embedder.js";
...
export function search({ query, queryEmbedding, entries, maxResults = 10, minSimilarity = 0.5, log }) {
  ...
  for (const entry of entries) {
    ...
    try {
      const score = cosineSimilarity(queryEmbedding, entry.embedding, queryNorm, entry.norm);
      scored.push({
        id: entry.id,
