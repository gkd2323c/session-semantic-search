...
export class SessionIndexer {
  ...
  async scanAll({ force } = {}) {
    ...
    for (const filePath of files) {
      ...
      if (chunks.length > 0) {
        ...
        this.#updateMeta(relPath, filePath, exchanges.length, chunks.length, exchanges);
        indexed++;
        this.#log?.info(`  indexed: ${relPath} → ${chunks.length} chunks`);
      } else {
        this.#updateMeta(relPath, filePath, exchanges.length, 0, exchanges);
        skipped++;
      }
    }
  ...
  async scanIncremental() {
    ...
    for (const filePath of files) {
      ...
      if (known && known.processedExchangeCount > 0 && currentExchanges.length > known.processedExchangeCount) {
        if (await this.#verifyPrefix(currentExchanges, known.processedExchangeCount, relPath)) {
          ...
          if (filteredChunks.length > 0) {
            ...
            this.#updateMeta(relPath, filePath, currentExchanges.length, (known.chunkCount || 0) + filteredChunks.length, currentExchanges);
            indexed++;
      ...
      if (allChunks.length > 0) {
        ...
        this.#updateMeta(relPath, filePath, currentExchanges.length, allChunks.length, currentExchanges);
        indexed++;
      } else {
        this.#updateMeta(relPath, filePath, currentExchanges.length, 0, currentExchanges);
        skipped++;
      }
    }
  ...
  async #embedAndStore(chunks, sessionId, isAppend = false) {
    ...
    // 内容寻址去重：为每个 chunk 计算 hash，只嵌入新的
    const newChunks = [];
    let deduped = 0;

    for (const chunk of chunks) {
      const hash = this.#contentHash(chunk.text);
      chunk.contentHash = hash;
      if (this.#store.hasContentHash(hash)) {
        deduped++;
      } else {
    ...
    if (newChunks.length === 0) {
      this.#log?.info(`  dedup: all ${chunks.length} chunks already indexed, skipping`);
      return;
    }

    if (deduped > 0) {
      this.#log?.info(`  dedup: ${newChunks.length}/${chunks.length} chunks new, ${deduped} skipped`);
    }

    const texts = newChunks.map(c => c.text.substring(0, 3000));

    // 批量嵌入（只为新 chunk 调用，节省 API 成本）
    const embeddings = await this.#embedder.embedBatch(texts);

    // 提取会话元信息
    const timestamp = newChunks[0]?.timestamp || new Date().toISOString();
    const title = timestamp.substring(0, 10);

    // 提取用户话题词
    const userTopics = this.#extractUserTopics(newChunks);

    // 存储（带 contentHash）
    const entries = newChunks.map((chunk, i) => ({
      sessionId,
